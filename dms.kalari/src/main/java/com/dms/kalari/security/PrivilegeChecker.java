...
public class PrivilegeChecker {
	...
	private final XorMaskHelper xorMaskHelper;

	private final HttpServletRequest request;

	@Autowired
	public PrivilegeChecker(AuthLoginLogService authLoginLogService, AuthLogActionService authLogActionService,
			XorMaskHelper xorMaskHelper, HttpServletRequest request) {
		this.authLoginLogService = authLoginLogService;
		this.authLogActionService = authLogActionService;
		this.xorMaskHelper = xorMaskHelper;
		...
		if (auth != null && auth.isAuthenticated() && auth.getPrincipal() instanceof CustomUserPrincipal u) {
			...
			if (loginId == null) {
				try {
					...
					loginId = newLoginId;
					// xorMaskHelper.initializeSessionKey(569 + newLoginId + 74L);

					String concatenated = "569" + newLoginId + "74";

					// Parse back to long
					long sessionKey = Long.parseLong(concatenated);

					// Initialize
					xorMaskHelper.initializeSessionKey(sessionKey);

					System.out.println("initializeSessionKey " + 569 + newLoginId + 74L);
				} catch (Exception ex) {
	...
	private boolean isPublicResource(String path) {
		...
		return cleanPath.startsWith("/public/") || cleanPath.equals("/login") || cleanPath.equals("/logout")|| cleanPath.equals("/auth-error")
				...
				|| cleanPath.startsWith("/webjars/") || cleanPath.startsWith("/static/")
				|| cleanPath.startsWith("/assets/") || cleanPath.equals("/favicon.ico") || cleanPath.equals("/health")
				|| cleanPath.equals("/actuator/health") || cleanPath.startsWith("/actuator/info");
	}
	...
	/**
	 * Transform alias to real path WITHOUT access checking Use this when access has
	 * already been verified by security filter
	 */
	...
	public Optional<String> getAliasForRealPath(String realPath) {
		...
		if (auth != null && auth.isAuthenticated() && auth.getPrincipal() instanceof CustomUserPrincipal user) {
			...
			return user.getPrivileges().stream().filter(alias -> {
